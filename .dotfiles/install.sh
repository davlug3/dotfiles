...
if command -v git >/dev/null 2>&1; then
	...
	curl -o "$(pwd)/install_git.sh" "http://cdn.jsdelivr.net/gh/$REPO_NAME/.dotfiles@git/install_git.sh" || {
...
if [ -f "$(pwd)/install_dotfiles.sh" ]; then
	...
	curl -O "http://cdn.jsdelivr.net/gh/$REPO_NAME/.dotfiles@git/install_dotfiles.sh" || {
	    echo "Failed to download install_dotfiles.sh";
	    exit 1;
	}
	echo "downloaded. $(pwd)"
	bash "install_dotfiles.sh" || {
	    echo "install_dotfiles.sh failed"
	    exit 1
	}
